    ...
    public partial class DuzenliNakitBagisciListesiWP : WebPart
    {
            ...
            if (at != null)
            {
                ...
                if (dataTable != null)
                {
                    foreach (DataRow row in dataTable.Rows)
                    {
                        ...
                        listItem.DuzenliBagisciBelgesi = DuzenliBagisciBelgesiLinkiGetir(aBagisciId, aArmaganId, aBaslamaTarihi, bBelgeIstemiyor, bDurum, aDuzenliBagisciId); //property içindeki logic çalistiriliyor
        ...
        private string BagisciEslestirLinkGetir(int aBagisciId, int aArmaganId, string aBagisciAdi, string bBagisciAdi, string bDurum, int aDuzenliBagisciId, string aEslesmeBilgisi)
        {
            ...
            if (!aEslesmeBilgisi.Contains("TCKN ile bulundu"))
            {
                ...
                {
                    link = "<a href='" + ProjeConstants.PAGE_NAKITBAGISCI_EDIT + "?NakitBagisciId=" + aBagisciId + "' class='btn btn-outline-secondary ' >Bagisçi</a> " +
        ...
        private string DuzenliBagisciBelgesiLinkiGetir(int aBagisciId, int aArmaganId, DateTime aBaslamaTarihi, bool bBelgeIstemiyor, string bDurum, int aDuzenliBagisciId)
        {
            ...
            if (aBagisciId == -1)
            {
                link = "Bagisçi bulunamadi";
            }
            else if (bBelgeIstemiyor == true)
            {
                link = "Belge Istemiyor";
            }
            else
            {
                ////Duzenli bagisçi belgesi olusturulmus mu, Armagan_Table'dan kontrol et
                ...
                {
                    link = "<button type=button class='btn btn-success' onclick=showDuzenliBagisOnay(" + aBagisciId + "," + aDuzenliBagisciId + ")>Düzenli Bagisçi Belgesi olustur</button>";
                }
            }
            return link;
        }
        ...
        protected void ArmaganOlusturBtn_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    Exception exception = new Exception(string.Format("HATA SATIRI {0}: Bagisçi bulunamadi -> NakitBagisciId: {1}", ProjeConstants.PAGE_DUZENLIBAGISCI_YUKLEME, nakitBagisciId));
        ...
        public ExceptionHelper ArmaganOlustur(DuzenliNakitBagisci duzenliNakitBagisci, NakitBagisci bagisci)
        {
            ...
            try
            {
                ...
                if (nakitBagisHareket != null)
                {
                    ...
                    //Bu bagisçi daha önce Düzenli Bagisçi belgesi almis mi bak
                    Armagan armagan = new Armagan();
                    armagan = armagan.SelectByBagisciIdAndArmaganTanimId(bagisci.Id, ProjeConstants.ARMAGAN_DUZENLIBAGISCIBELGESIID);
                    if (armagan != null)
                    {
                        string aciklamaStr = string.IsNullOrEmpty(duzenliNakitBagisci.Aciklama) ? " Daha önce Düzenli Bagisçi Belgesi almis -> " + duzenliNakitBagisci.BagisciAdi : duzenliNakitBagisci.Aciklama;
                        Exception exception = new Exception(string.Format("HATA SATIRI {0}:{1} -> Bu bagisçi daha önce Düzenli Bagisçi Belgesi almis.", ProjeConstants.PAGE_DUZENLIBAGISCI_YUKLEME, aciklamaStr));
                    ...
                    if (armaganId < 1)
                    {
                        string aciklamaStr = string.IsNullOrEmpty(duzenliNakitBagisci.Aciklama) ? " Armagan kaydi olusturulamadi -> " + duzenliNakitBagisci.BagisciAdi : duzenliNakitBagisci.Aciklama;
                        Exception exception = new Exception(string.Format("HATA SATIRI {0}:{1} -> Armagan kaydi olusturulamadi.", ProjeConstants.PAGE_DUZENLIBAGISCI_YUKLEME, aciklamaStr));
                        exceptionHelper.Exceptions.Add(exception);
                    }
                    else
                    {
