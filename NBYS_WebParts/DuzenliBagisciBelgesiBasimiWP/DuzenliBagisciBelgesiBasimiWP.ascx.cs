...
namespace NBYS_WebParts.DuzenliBagisciBelgesiBasimiWP
{
    ...
    public partial class DuzenliBagisciBelgesiBasimiWP : WebPart
    {
        ...
        private void TabloOlustur()
        {
            ...
            {
                TableDataLbl.Text = "Düzenli Bagisçi Belgesi bulunmamaktadir.";
        ...
        private string GetData()
        {
            ...
            TableDataLbl.Text = rowCount + " adet Düzenli Bagisçi Belgesi mevcut";
            if (rowCount > 0)
            {
                DosyaOlusturBtn.Visible = true;
                AdresOlusturBtn.Visible = true;
        ...
        protected void DosyaOlusturBtn_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    MessageHelper.PublishMessage("Düzenli Bagisçi belgeleri hazirlandi, Dosya ismine basarak yaziyi indirebilirsiniz.", ProjeConstants.MESAJ_BASARILI, 2000);
                }
                else
                {
                    MessageHelper.PublishMessage("Düzenli Bagisçi belgeleri olusturulamadi.", ProjeConstants.MESAJ_HATA, 3000);
                }
            }
            catch (Exception ex)
            {
        ...
        private MemoryStream AddData2DestinationStream(MemoryStream templateStream, IEnumerable<Paragraph> templateParagraphs)
        {
            ...
            if (dataTable != null)
            {
                foreach (DataRow row in dataTable.Rows)
                {
                    ...
                    // Dokümanin sonuna bos sayfa yani fazladan sayfa atiyor
