            ...
            try
            {
                if (NakitBagisciIdQS.ConvertToInt()>0)
                {
                    ...
                    else
                    {
                        ...
                        MessageHelper.PublishMessage("Bagisçi bölgenizde olmadigindan bilgilerine erisemezsiniz", ProjeConstants.MESAJ_HATA);
                    }
                }
                else
                {
        ...
        protected void KaydetBtn_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (string.IsNullOrWhiteSpace(AdiTxt.Text))
                {
                    ...
                    //    typeof(System.Web.UI.Page), System.Guid.NewGuid().ToString(), "Alert.danger('Bagisçi Adi bos olamaz. Lütfen bagisçi adini giriniz.')", true);
                    MessageHelper.PublishMessage("Bagisçi Adi bos olamaz. Lütfen bagisçi adini giriniz.", ProjeConstants.MESAJ_HATA);
                }
                else
                {
                    int nakitBagisciId = NakitBagisciIdQS.ConvertToInt();
                ...
                if (isSaved)
                {
                    //System.Web.UI.ScriptManager.RegisterStartupScript((System.Web.UI.Page)System.Web.HttpContext.Current.Handler, typeof(System.Web.UI.Page), System.Guid.NewGuid().ToString(), "Alert.success('Nakit Bagisçi Bilgisi Kaydedildi.')", true);
                    MessageHelper.PublishMessage("Nakit Bagisçi Bilgisi Kaydedildi.", ProjeConstants.MESAJ_BASARILI, 4000);

                }
                else
                {
                    //System.Web.UI.ScriptManager.RegisterStartupScript((System.Web.UI.Page)System.Web.HttpContext.Current.Handler, typeof(System.Web.UI.Page), System.Guid.NewGuid().ToString(), "Alert.danger('Nakit Bagisçi Bilgisi Kaydedilemedi.')", true);
                    MessageHelper.PublishMessage("Nakit Bagisçi Bilgisi Kaydedilemedi.", ProjeConstants.MESAJ_HATA);
                }
            }
            catch (Exception ex)
            {
