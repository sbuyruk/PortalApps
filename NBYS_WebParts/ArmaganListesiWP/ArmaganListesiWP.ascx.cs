    ...
    public partial class ArmaganListesiWP : WebPart
    {
        ...
        private string GetModalDataJson(string nakitBagisciId)
        {
            ...
            BagisBilgileriLbl.Text = rowCount < 1 ? "Bagis bulunmamaktadir" :
                "Bagisçinin " + ProjeConstants.BAGIS_SORGU_BASTAR.ConvertToDatetime()+ " tarihinden itibaren "+ rowCount + " defada yaptigi toplam " + toplamTutar.ToString("N", culturInfo) + "TL bagisi bulunmaktadir";
            return json;
        }
            ...
            if (yetkiliMi)
            {
                belgeStr = @"
                    if(row.Durum.toString().indexOf('Kontrol Edildi')>=0){
                        if(row.ArmaganBaslik.toString().indexOf('Tesekkür Belgesi')>=0)
                        {
                            ...
                            row.ArmaganBaslik.toString().indexOf('Gümüs Madalya ve Berati')>=0 || 
