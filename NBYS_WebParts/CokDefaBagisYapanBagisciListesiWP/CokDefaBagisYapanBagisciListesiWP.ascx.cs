...
namespace NBYS_WebParts.CokDefaBagisYapanBagisciListesiWP
{
    ...
    public partial class CokDefaBagisYapanBagisciListesiWP : WebPart
    {
            ...
            if (at != null)
            {
                ...
                if (dataTable != null)
                {
                    foreach (DataRow row in dataTable.Rows)
                    {
                        ...
                        int oncelik = 9;
                        listItem.Madalya = MadalyaLinkiGetir(nakitBagisciId, toplamBagisAdedi, toplamBagisTutari, tuzelKisi, sonBagisTarihi, out oncelik);
                        listItem.Oncelik = oncelik;
                        list.Add(listItem);
                    }
                }
            }
        ...
        private string MadalyaLinkiGetir(int nakitBagisciId, string toplamBagisAdedi, decimal toplamBagisTutari, bool tuzelKisiMi, DateTime sonBagisTarihi,  out int oncelik)
        {
            ...
            if (hakedilenArmaganTanim != null)
            {
                ...
                {
                    //Altin Hakediyor
                    //Altin Madalya almis mi
                    Armagan aldigiAltin = aldigiArmaganlar.FirstOrDefault(arm => arm.ArmaganTanimId == ProjeConstants.ARMAGAN_ALTINID);

                    if (aldigiAltin == null)
                    {
                        oncelik = 1;
                        //Altin madalya almamis ama genel toplamda hak etms
                        retval = "<a href=# onclick=\"OpenArmaganModal(" + nakitBagisciId + "," + hakedilenArmaganTanim.Id + ",\'" + sonBagisTarihi + "\');\" class=\'btn btn-outline-warning \'>Altin Madalya Olustur</a>";
                    }
                    else
                    {
                        oncelik = 6;
                        retval = aldigiAltin.Tarih.ConvertToDatetimeEmptyIfNull() + " Tarihinde " + " Altin Madalya Verilmistir. ( Durumu :" + aldigiAltin.Durum + ")";
                    }
                ...
                else if (hakedilenArmaganTanim.Id == ProjeConstants.ARMAGAN_GUMUSID)
                {
                    ...
                    {
                        //Gümüs madalya almamis ama genel toplamda hak etms
                        //retval = "<a href=" + ProjeConstants.PAGE_SIRADISIARMAGAN_DUZENLE + @"?DestinationApp=BD&NakitBagisciId=" + nakitBagisciId + " class=\'btn btn-outline-secondary \'>Gümüs Madalya Olustur</a>";
                        oncelik = 2;
                        retval = "<a href=# onclick=\"OpenArmaganModal(" + nakitBagisciId + "," + hakedilenArmaganTanim.Id + ",\'" + sonBagisTarihi + "\');\" class=\'btn btn-outline-secondary \'>Gümüs Madalya Olustur</a>";
                    }
                    else
                    {
                        oncelik = 7;
                        retval = aldigiMadalya.Tarih.ConvertToDatetimeEmptyIfNull() + " Tarihinde " + " Gümüs Madalya Verilmistir. ( Durumu :" + aldigiMadalya.Durum + ")";
                    }
                ...
                {
                    //Bronz Hakediyor
                    //Bronz Madalya almis mi
                    Armagan aldigiMadalya = aldigiArmaganlar.FirstOrDefault(arm => arm.ArmaganTanimId == ProjeConstants.ARMAGAN_BRONZID);

                    if (aldigiMadalya == null)
                    {
                        //Bronz madalya almamis ama genel toplamda hak etms
                        oncelik = 3;
                        retval = "<a href=# onclick=\"OpenArmaganModal(" + nakitBagisciId + "," + hakedilenArmaganTanim.Id + ",\'" + sonBagisTarihi + "\');\" class=\'btn btn-outline-info \'>Bronz Madalya Olustur</a>";
                    }
                    else
                    {
                        oncelik = 8;
                        retval = aldigiMadalya.Tarih.ConvertToDatetimeEmptyIfNull() + " Tarihinde " + " Bronz Madalya Verilmistir. ( Durumu :" + aldigiMadalya.Durum + ")";
                    }
        ...
        private int MadalyaOlustur(int nakitBagisciId, int hakedilenArmaganTanimId, DateTime sonBagisTarihi)
        {
            //Bagisciyi bul
            //NakitBagisHareketten bagis listesini ve bagis toplamini al
            ...
            if (nakitBagisci != null)
            {
                ...
                if (nakitBagisHareketListesi.Count > 0)
                {
                    ...
                    {
                        MessageHelper.PublishMessage("Bagisçinin armagan hakki bulunmamaktadir.", ProjeConstants.MESAJ_HATA);
                    }
                }
            }
            else
        ...
        #region Verilen Armaganlar
        private void TabloOlusturVerilenArmagan()
        {
            List<BagisciListItem> list = GetVerilenArmaganData();
            var serializer = new JavaScriptSerializer();
            var jsonData = serializer.Serialize(list);
            var jsString = CreateDataTableVerilen(jsonData); //javascript kodu hazirlaniyor.
            UtilityHelper.ScriptCalistir(jsString);
        }
        ...
        protected void ArmaganOlusturModalDoldurBtn_Click(object sender, EventArgs e)
        {
            ...
            if (nakitBagisci != null)
            {
                ...
                if (armaganTanim != null)
                {
                    ...
                    ArmaganOlusturMessageTxt.Text = nakitBagisci.Adi + " " + nakitBagisci.Soyadi + " adli bagisçi için " + armaganTanim.Armagan + " olusturulmasini istiyor musunuz?";
                }
                else
                {
                    ArmaganOlusturNowBtn.Visible = false;
                    ArmaganOlusturMessageTxt.Text = nakitBagisci.Adi + " " + nakitBagisci.Soyadi + " adli bagisçi için Armagan tanimi bulunamadigindan armagan berati olusturulamiyor";
                }
            }
            else
            {
                ArmaganOlusturNowBtn.Visible = false;
                ArmaganOlusturMessageTxt.Text = hiddenNakitBagisciId.Value + " numarali bagisçi armagan berati olusturulamiyor";
            }
        }
        protected void ArmaganOlusturNowBtn_Click(object sender, EventArgs e)
        { 
            int nakitBagisciId=hiddenNakitBagisciId.Value.ConvertToInt();
            DateTime sonBagisTarihi=hiddenSonBagisTarihi.Value.ConvertToDatetime();
            try
            {
                int armaganId = MadalyaOlustur(hiddenNakitBagisciId.Value.ConvertToInt(), hiddenArmaganTanimId.Value.ConvertToInt(), hiddenSonBagisTarihi.Value.ConvertToDatetime());
                if (armaganId > 0)
                    RedirectToPage(ProjeConstants.PAGE_ARMAGAN_EDIT + "?ArmaganId=" + armaganId + "&NakitBagisciId=" + hiddenNakitBagisciId.Value.ConvertToInt());
                else
                    MessageHelper.PublishMessage("Armagan Olusturulamadi", ProjeConstants.MESAJ_HATA);
            }
