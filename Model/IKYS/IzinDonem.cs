...
namespace Model.IKYS
{
    public class IzinDonem : ParentClass
    {
        ...
        private string SelectByIzinTarihiSQL(int personelId, int izinTipi, DateTime tarih)
        {
            ...
            string sqlstr = string.Format(@" 
                    ...
                    WHERE PersonelId={0} {1}
                        AND (BaslangicTarihi<={2} AND BitisTarihi>{3}) ----AND BitisTarihi>={3}) izindönemnin son gününün ertesi günü yeni izin dönemi olmalı SB 01.06.2020 
        ...
        /// <summary>
        /// personel,izinTipi ve tarih bazında IzinDonem_Table'da kayıt yoksa, IzinDonemi tipinde kayıt ekler,  
        /// IzinDonemi tipinde eklediği yeni nesneyi döndürür.
        /// </summary>
        ...
        public IzinDonem IzinDonemiOlustur(Personel personel, int izinTipi, DateTime tarih, string currentUserName)
        {
            ...
            if (!string.IsNullOrEmpty(izinDonemiBasTarStr))
            {
                ...
                if (izinDonemi == null)
                {
                    ...
                    izinDonemi.Adi = izinDonemiBasi.Year + "-" + izinDonemiSonu.Year + " İzin Donemi";
        ...
        {
            //bir kişinin izin dönemi belirlendikten sonra 
            IzinDonem izinDonemi = null;
            IsBilgileri ib = new IsBilgileri();
            ib = ib.SelectByPersonelId(personel.Id);
            string baslamaTarStr = ib == null ? "" : ib.BaslamaTar.ConvertToDatetimeEmptyIfNull();
            ...
            if (!string.IsNullOrEmpty(izinDonemiBasTarStr))
            {
                ...
                if (izinDonemi != null)
                {
                    ...
                    if (izinDonemi.IzinTipi == ProjeConstants.IZINTIPI_UCRETLI_INT) // izin tipi ÜCRETLİ izinse 
                    {
