    ...
    public partial class KiraOdemeDosyasiYuklemeWP : WebPart
    {
                ...
                if (vakifBank2Data != null)
                {
                    foreach (DataRow row in vakifBank2Data.Rows)
                    {
                        ...
                        if (string.IsNullOrEmpty(hesapNo))// ilk kolon bos ise dosya bitti çik
                        {
                           ...
                           detay.Contains("TÜRK SILAHLI KUVVETLERINI GÜÇLENDIRME VAKFI tarafindan TÜRK SILAHLI KUVVETLERINI GÜÇLENDIRME VAKFI tarafina gelen")//ziraat bank
                           || detay.Contains("TÜRK SILAHLI KUVVETLERINI GÜÇLENDIRME V tarafindan TÜRK SILAHLI KUVVETLERINI GÜÇLENDIRME VAKFI tarafina gelen") //isbank
                           || detay.Contains("TÜRK SILAHLI KUVVETLERINI GÜÇL tarafindan TÜRK SILAHLI KUVVETLERINI GÜÇLENDIRME VAKFI tarafina gelen ")//garanti bank
                           || detay.Contains(" virman ")
                           || detay.Contains(" VIRMAN ")
                           || hesapNo.Equals("HESAP NO")
                           || tutar < 0) //Bu borç demek degil mi?
                        {
                            continue;
                        }
                        ...
                        try
                        {
                            ...
                            adi = adi.ToUpper().Replace("KUVEYT TÜRK KATILIM BANKASI A.S.", "")
                                   .Replace("QNB FINANSBANK A.S.", "")
                                   .Replace("YAPI VE KREDI BANKASI A.S.", "")
                                   .Replace("TÜRKIYE GARANTI BANKASI A.S.", "")
                                   .Replace("AKBANK T.A.S.", "")
                                   .Replace("TÜRKIYE GARANTI BANKASI A.S.", "")
                                   .Replace("TÜRKIYE IS BANKASI A.S.", "")
                                   .Replace("TÜRKIYE CUMHURIYETI ZIRAAT BANKASI A.S.", "")
                                   .Replace("TÜRKIYE HALK BANKASI A.S.", "")
                                   .Replace("TÜRK EKONOMI BANKASI A.S.", "").Trim();
                            ...
                            {
                                Exception kayitVar = new Exception("Islem Numarasi Çakisiyor. (" + adi + ", " + tutar + "TL, " + hareketTar);
                                exceptionHelper.Exceptions.Add(kayitVar);
                                continue;
                            }
                            int kiraciId = 0;
                            if (true)// Borç Alacak Ayrimi için
                            {
                                   ...
                                   )
                                {
                                    ...
                                    {
                                        // SB DosyaNo ile konu çözülemiyor! o yüzden kapandi
        ...
        private int OdemeSebebiBelirle(KiraEkstreAktarma kiraEkstreAktarma, string islem)
        {
               ...
               islem.ToLower(cultureInfo).Contains("Batch Komisyonu"))
            {
               ...
               islem.ToUpper(cultureInfo).Contains("OTOMATIK SÜPÜRME ISLEMLERI VIRMAN") ||
               islem.ToLower(cultureInfo).Contains("Otomatik Süpürme Islemleri Virman"))
            {
               ...
               islem.ToUpper(cultureInfo).Contains("VALOR ISLEMI IÇIN PARA ÇEK VE YATIR") ||
               islem.ToLower(cultureInfo).Contains("Valor Islemi Için Para Çek ve Yatir"))
            {
            ...
            else if (kiraEkstreAktarma != null && kiraEkstreAktarma.Aciklama != null)
            {
                if ((kiraEkstreAktarma.Aciklama.Contains(ProjeConstants.ODEMESEBEBI_GECICITEMINAT)) ||
                        kiraEkstreAktarma.Aciklama.ToUpper(cultureInfo).Contains("GEÇICI TEMINAT") ||
        ...
        private int KiraciOtomatikEslestir(string adi)
        {
            ...
            {
                //kiraEkstreAktarma Listesinde kiraciId>0 olan kayitlari ada göre sorgulasin 
                KiraEkstreAktarma keaDao = new KiraEkstreAktarma();
                ...
                //kiraci tablosundaki kayitlari isme göre sorgulasin
                if (kiraciId < 1)
                {
        ...
        private bool BirdenCokSozlesmesiVarMi(string adi)
        {
            ...
            if (!string.IsNullOrEmpty(adi))
            {
                //kiraSozlesme tablosundaki kayitlari isme göre sorgulasin
