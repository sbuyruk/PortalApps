    ...
    public partial class DereceKademeListesiWP : WebPart
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!Page.IsPostBack)
            {
                ...
                FillPersonelDDL();
                PersonelIdQS = string.IsNullOrEmpty(PersonelIdQS) ? PersonelDDL.SelectedItem.Value : PersonelIdQS; // Eger PersonelIdQS bos ise, geçerli kullanici Id'sini kullan
            ...
            else
            {
                ...
                if (dataTable!=null)
                {
                    foreach (DataRow row in dataTable.Rows)
                    {
                        ...
                        string degisimTarihi = ((DateTime)row["DegisimTarihi"]).ToString("yyyy-MM-dd"); // JS için uygun tarih formati

                        // JS parametrelerini güvenli hale getir (tirnak içine al)
        ...
        private void UpdateDereceKademeDegisim(int id, string degisim, DateTime degisimTarihi, int derece, int kademe, string aciklama)
        {
            ...
            try
            {
                ...
                MessageHelper.PublishMessage("Derece ve Kademe Degisikligi Güncellendi.", ProjeConstants.MESAJ_BASARILI);
            }
            catch (Exception)
            {
                MessageHelper.PublishMessage("Derece ve Kademe Degisikligi Güncellenemedi.", ProjeConstants.MESAJ_HATA);
            }
        }
        ...
        protected void KaydetBtn_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageHelper.PublishMessage("Kayit yapabilmek için geçerli derece ve kademeden farkli bir deger seçmelisiniz.", ProjeConstants.MESAJ_HATA);
            }
            else
            {
        ...
        protected void GuncelleBtn_Click(object sender, EventArgs e)
        {
            ...
            // Veriyi güncelleme islemi yapilacak
            // Örnek: Veritabanina veya SharePoint listesine veri güncelleme
